...
function requirePackageJson() {
  ...
  function findUp(fileName, cwd) {
    ...
    while (true) {
      ...
      if (fs2.existsSync(path$12.join(currentPath, fileName))) {
  ...
  class NodeExternalApi {
    ...
    getAppLogPath(appName = this.getAppName()) {
      if (this.platform === "darwin") {
        return path$12.join(this.getSystemPathHome(), "Library/Logs", appName);
      }
      return path$12.join(this.getAppUserDataPath(appName), "logs");
    }
    getAppName() {
      var _a;
    ...
    getAppUserDataPath(appName = this.getAppName()) {
      return appName ? path$12.join(this.getSystemPathAppData(), appName) : void 0;
    }
    getAppVersion() {
      var _a;
    ...
    getSystemPathAppData() {
      ...
      switch (this.platform) {
        case "darwin": {
          return path$12.join(home, "Library/Application Support");
        }
        case "win32": {
          return process.env.APPDATA || path$12.join(home, "AppData/Roaming");
        }
        default: {
          return process.env.XDG_CONFIG_HOME || path$12.join(home, ".config");
        }
      }
    }
    getSystemPathHome() {
  ...
  class ElectronExternalApi extends NodeExternalApi {
    ...
    isDev() {
      ...
      if (typeof process.execPath === "string") {
        const execFileName = path$12.basename(process.execPath).toLowerCase();
...
function requireFormat() {
  if (hasRequiredFormat) return format$7;
  hasRequiredFormat = 1;
  const { transform } = requireTransform();
  format$7 = {
...
function requireFileRegistry() {
  ...
  class FileRegistry extends EventEmitter {
    ...
    testFileWriting({ filePath, writeOptions }) {
      fs2.mkdirSync(path$12.dirname(filePath), { recursive: true });
  ...
  function fileTransportFactory(logger, { registry = globalRegistry, externalApi } = {}) {
    ...
    return Object.assign(transport, {
      ...
      archiveLogFn(file2) {
        ...
        const inf = path$12.parse(oldPath);
        try {
          fs2.renameSync(oldPath, path$12.join(inf.dir, `${inf.name}.old${inf.ext}`));
        } catch (e) {
      ...
      resolvePathFn(vars) {
        return path$12.join(vars.libraryDefaultDir, vars.fileName);
      },
      setAppName(name) {
    ...
    function readAllLogs({ fileFilter = (f) => f.endsWith(".log") } = {}) {
      initializeOnFirstAccess();
      const logsPath = path$12.dirname(transport.resolvePathFn(pathVariables));
      if (!fs2.existsSync(logsPath)) {
        return [];
      }
      return fs2.readdirSync(logsPath).map((fileName) => path$12.join(logsPath, fileName)).filter(fileFilter).map((logPath) => {
        try {
          return {
            path: logPath,
...
const isDev = process.env.NODE_ENV === "development" || !electron.app.isPackaged && !process.execPath.includes("release");
...
function getYtDlpPath() {
  const appDir = isDev ? path.join(process.cwd(), "resources") : path.join(__dirname, "..");
  const bundledPath = path.join(appDir, "yt-dlp.exe");
  if (require$$0$1.existsSync(bundledPath)) {
...
electron.ipcMain.handle("check-yt-dlp", async () => {
  return new Promise((resolve2) => {
    ...
    const tryCommand = (index) => {
      ...
      const proc = require$$0$2.spawn(cmd, args, { shell: true });
      proc.on("close", (code2) => {
        if (code2 === 0) {
...
electron.ipcMain.handle("install-yt-dlp", async () => {
  return new Promise((resolve2, reject) => {
    const proc = require$$0$2.spawn("python", ["-m", "pip", "install", "yt-dlp", "--upgrade", "--user"], { shell: true });
    let output = "";
    ...
    proc.on("close", (code2) => {
      if (code2 === 0) {
        ...
        resolve2("yt-dlp installed successfully");
      } else {
        log.info("pip install failed, trying winget...");
        const wingetProc = require$$0$2.spawn("winget", ["install", "yt-dlp.yt-dlp", "-e", "--source", "winget"], { shell: true });
        wingetProc.on("close", (wingetCode) => {
          if (wingetCode === 0) {
            resolve2("yt-dlp installed via winget");
          } else {
...
electron.ipcMain.handle("get-video-info", async (_, url) => {
  return new Promise((resolve2, reject) => {
    const ytDlpPath = getYtDlpPath();
    ...
    const proc = require$$0$2.spawn(cmd, procArgs, { shell: true });
    let output = "";
    let errorOutput = "";
    proc.stdout.on("data", (data) => {
      output += data.toString();
    ...
    proc.on("close", (code2) => {
      if (code2 === 0 && output) {
        try {
          ...
          const title2 = json.title || "Unknown";
...
electron.ipcMain.handle("get-formats", async (_, url) => {
  return new Promise((resolve2) => {
    const ytDlpPath = getYtDlpPath();
    ...
    proc.on("close", (code2) => {
      if (code2 === 0) {
        ...
        for (const line of lines) {
          ...
          for (const pattern2 of extPatterns) {
          ...
          for (const pattern2 of resPatterns) {
            const match = line.match(pattern2);
            if (match) {
              resolution = match[1];
              break;
        ...
        const sortedFormats = formats2.sort((a, b) => {
        ...
        resolve2(sortedFormats.slice(0, 50));
      } else {
...
electron.ipcMain.handle("download-video", async (_, options) => {
  const { url, formatId, outputDir, trimStart, trimEnd } = options;
  url.toLowerCase().includes("twitter.com") || url.toLowerCase().includes("x.com");
  return new Promise((resolve2, reject) => {
    const ytDlpPath = getYtDlpPath();
    let args;
    ...
    const cmd = ytDlpPath.includes("yt-dlp") && !ytDlpPath.endsWith(".exe") ? "python" : ytDlpPath;
    const procArgs = ytDlpPath.includes("yt-dlp") && !ytDlpPath.endsWith(".exe") ? ["-m", "yt_dlp", ...args] : args;
    const proc = require$$0$2.spawn(cmd, procArgs, {
      shell: true,
    ...
    proc.on("close", (code2) => {
      if (code2 === 0) {
        ...
        resolve2("Download completed successfully");
      } else {
        log.error("Download failed with code:", code2);
        reject(new Error(`Download failed with code ${code2}`));
      }
    });
    proc.on("error", (err) => {
...
electron.ipcMain.handle("download-from-queue", async (_, item) => {
  ...
  return new Promise((resolve2, reject) => {
    ...
    proc.on("close", (code2) => {
      if (code2 === 0) {
        ...
        mainWindow == null ? void 0 : mainWindow.webContents.send("queue-item-complete", { itemId: item.url, success: true });
        resolve2(finalFilePath || "Download completed");
      } else {
        log.error("Queue download failed:", code2);
        mainWindow == null ? void 0 : mainWindow.webContents.send("queue-item-complete", { itemId: item.url, success: false, error: `Failed with code ${code2}` });
        reject(new Error(`Download failed with code ${code2}`));
      }
    });
    proc.on("error", (err) => {
      log.error("Queue download error:", err);
      mainWindow == null ? void 0 : mainWindow.webContents.send("queue-item-complete", { itemId: item.url, success: false, error: err.message });
      reject(err);
    });
  });
});
...
electron.ipcMain.handle("get-file-size", async (_, filePath) => {
  try {
    const stats = require$$0$1.statSync(filePath);
    return stats.size;
  } catch (err) {
...
electron.ipcMain.handle("add-to-history", async (_, item) => {
...
electron.ipcMain.handle("remove-from-history", async (_, id2) => {
  const history = store.get("history", []);
  const updated = history.filter((item) => item.id !== id2);
  store.set("history", updated);
  log.info("Removed from history:", id2);
});
electron.ipcMain.handle("add-to-favorites", async (_, item) => {
...
electron.ipcMain.handle("remove-from-favorites", async (_, id2) => {
  const favorites = store.get("favorites", []);
  const updated = favorites.filter((item) => item.id !== id2);
  store.set("favorites", updated);
  log.info("Removed from favorites:", id2);
});
electron.ipcMain.handle("is-favorite", async (_, url) => {
  const favorites = store.get("favorites", []);
